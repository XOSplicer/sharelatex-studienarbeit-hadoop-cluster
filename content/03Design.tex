...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Create internal network and security groups which allow access for port 22 (\ac{SSH}) and 8080 for web access to Ambari.
        \item Create an appropriate number of maximum sized \acp{VM} that are connected to both the internal and external network. Name them according to the names in \ref{fig:architecture}. Assign the security group to them.
        \item 
        
    \end{enumerate}
    TODO Fist create inner network and security groups, then appropriate number of VMs according to \ref{fig:architecture}, at least 3, names accordingly. ATTENTION: Network connection: due to a bug in openstack first only outer network connection then attach inner connection and reboot. note down the internal and external ips displayed by openstack, note down the eth port that is newly created for internal connection. (maybe use enumeration here and in other items)
