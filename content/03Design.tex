...
\begin{enumerate}
    ...
    \ietm On the slave nodes only the Ambari agent is installed.
    \item on the master the initial Ambari server setup is performed which
        installs an \ac{JDK} 
        and starts the Ambari server service as the root user.
\end{enumerate}

Now the Hadoop cluster c
