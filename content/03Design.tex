...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \texttt{slaves} -- This file is used on the master host to identify all the hosts that run an nodemanager or an datanode.
        Therefore the \acs{FQDN} of the master and the salve host are entered here.
        \item \texttt{yarn-site.xml} -- This file describes the settings for \ac{YARN}. Here the master's \ac{FQDN} is entered as the resourcemanager and \ac{YARN} is bound to 0.0.0.0. Furthermore the minmal allocation units for \ac{CPU} cores as well as \ac{RAM} and their res
    \end{itemize}
\end{enumerate}
