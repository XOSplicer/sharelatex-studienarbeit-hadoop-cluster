...
Further test included the execution \emph{teragen} and \emph{terasort} \autocite[][]{omally2008terasort}, which generate a variable amount of numbers and sorts them, in this case 5 \ac{GB} of data. 
...
TODO: describe that a ansible playbook is written for that

\paragraph{Ansible}
\emph{Ansible} is an configuration management tool.
...
[...]
[It] is written entirely in Python [and it] works by running commands via SSH, so there’s no need to install any server software.}
\autocite[][Chap. 1]{heap2016ansible}

Alternatives are \emph{Puppet} and \emph{Chef}, which both use a central server to distribute the 
...
The wizards now installs the cluster automagically.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Configure the \texttt{inventory.ini} file which describes the Ansible inventory. Supply the internal \ac{IP} address as well as the previously noted Ethernet adapter of the node. The \emph{hadoop-master-0} node should be listed in the \emph{master} section, all others in the \emph{slave} section.
        \item Prepare the nodes for the usage with Ansible by running\\
        \texttt{\$ run\_host\_setup.sh <host>} for each of them.
        \item Apply the Ansible playbook to the host by running\\
        \texttt{\$ ansible-playbook all.yml}\\
        This configures the host systems in such a way that Ambari Server and Client resp. are installed on them.
    \end{enumerate}
    
    TODO ("must still be implemened, has been made ofr example"),
    ...
    \item \textbf{Test Usage} 
        \begin{enumerate}
            ...
            \item TODO test with teragen and terasort \urlinline{https://hadoop.apache.org/docs/r1.0.4/api/org/apache/hadoop/examples/terasort/TeraGen.html}\\urlinline{https://www.systutorials.com/3235/hadoop-terasort-benchmark/} (also mentioned in \autocite[][]{white2015hadoop})
