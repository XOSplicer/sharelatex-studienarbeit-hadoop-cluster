...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item Prepare the nodes for the usage with Ansible by running \texttt{\$ run\_host\_setup.sh <host>} for each of them.
        \item Apply the Ansible playbook to the host by running \texttt{\$ ansible-playbook all.yml}. 
    \end{enumerate}
    
    ("must still be implemened, has been made ofr example")TODO,
     (in implementation explain how the ansible part works). finally role out the configuration by running ansible command
