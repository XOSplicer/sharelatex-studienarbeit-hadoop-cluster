...
OpenStack in Mannheim data center
what is OpenStack, how does it do clustering, what are possibilities, limitations of compute, store and network
(OpenStack general and dhbw)
limited resources!!!!
...
Use cases:
e.g. TODO
Philipps paper focuses on those use cases.
Main Focus: Usability for lecture use
Easily and reproducible to deploy which implies automation

requirements are also covered by the WI paper with
extensive results and comparisons \autocite{wi2018managementsystems}
 > runs on Mannheim OpenStack
 > low or no costs
 > fast deployment
 > student use either for learning the deployment or learning usage of Hadoop for analysis
 > support for recent additional tools
 > scaling on cluster
...
For this purpose the OpenStack environment \emph{BWCloud} (\url{https://www.bw-cloud.org/de/projekt}) 
...
Ubuntu Server (\url{https://www.ubuntu.com/server}) 16.04.3 \ac{LTS} is used as the operating system. 
...
\begin{enumerate}
    \item The system is updated and OpenJDK 1.8 (\url{http://openjdk.java.net/})  is installed using\texttt{apt}. 
...
\begin{enumerate}
    \item First Hadoop is downloaded from the Apache website (\url{https://hadoop.apache.org/releases.html}). 
...
\begin{enumerate}
    \item \textbf{\ac{VM} Creation} TODO
    \item \textbf{System Set-Up with Ansible} TODO
    \item \textbf{Hadoop Set-Up with Ambari} TODO
    \item \textbf{Test Usage} TODO
\end{enumerate}
