...
For each implementation detail the specific values chosen are given during the process. Each of the steps holds 
possibilities for errors which will also be evaluated.
...
The names are respective to the proposed architecture but have a \emph{dhbw} domain ending 
...
In order for the Hadoop cluster to store \ac{HDFS} data it is necessary to split up the available 1000~\ac{GB} for storage into three volumes of 330~\ac{GB}, one for each host.
...
To specify which actions are needed for the system configuration, a playbook is created which is split into three roles: \emph{common, master} and \emph{slave}.
...
Ambari provides a default preset for each service.
...
This holds the possibility for configuration errors which lead to the failure of the installation at first.
...
To run the test the following commands are used to create, sort and validate 5~\ac{GB} of data:
...
However the general concept of using OpenStack to host the cluster and Ansible and Ambari to deploy the cluster software and manage Hadoop is feasible and has clearly advantages above a manual cluster management.
