...
The roles are assigned in the inventory description, where also host specific variables are configured.  Those variables are later determined, as shown in section \ref{sec:impl:ansible:exec}. The variables are used to fill so-called \emph{templates} which are files with placeholders, that are used throughout the system.
...
\begin{enumerate}
    \item Install the \emph{ambari-server} package wich contains all the binaries necessary to run Ambari server.
